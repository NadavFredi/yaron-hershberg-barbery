...
const CONFIG: Record<Variant, {
    tableName: "dog_types" | "dog_categories"
    relationTable: "breed_dog_types" | "breed_dog_categories"
    relationColumn: "dog_type_id" | "dog_category_id"
    title: string
    description: string
    ...
    category1: {
        tableName: "dog_types",
        relationTable: "breed_dog_types",
        relationColumn: "dog_type_id",
    ...
    category2: {
        tableName: "dog_categories",
        relationTable: "breed_dog_categories",
        relationColumn: "dog_category_id",
...
interface DogCategoriesManagementPageProps {
    variant: Variant
}

export default function DogCategoriesManagementPage({ variant }: DogCategoriesManagementPageProps) {
    ...
    const fetchCategories = useCallback(async () => {
        console.log("🔍 [DogCategoriesManagementPage] Fetching categories", { variant })
        setIsLoading(true)
        try {
            ...
            console.log("✅ [DogCategoriesManagementPage] Loaded categories", { variant, mapped })

            setItems(mapped)
        } catch (error) {
            console.error("❌ [DogCategoriesManagementPage] Failed to fetch categories", { variant, error })
            toast({
                title: "שגיאה בטעינה",
    ...
    const handleDialogSubmit = async (name: string) => {
        try {
            ...
            if (dialogMode === "create") {
                console.log("📝 [DogCategoriesManagementPage] Creating category", { variant, name })
                ...
                console.log("📝 [DogCategoriesManagementPage] Updating category", { variant, id: selectedItem.id, name })
            ...
            console.error("❌ [DogCategoriesManagementPage] Failed saving category", { variant, error })
            toast({
                title: "שגיאה בשמירה",
    ...
    const handleDelete = async () => {
        ...
        try {
            setIsSaving(true)
            console.log("🗑️ [DogCategoriesManagementPage] Deleting category", { variant, id: selectedItem.id })
            ...
            console.error("❌ [DogCategoriesManagementPage] Failed deleting category", { variant, error })
            toast({
                title: "שגיאה במחיקה",
