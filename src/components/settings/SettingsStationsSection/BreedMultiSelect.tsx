...
export interface BreedMultiSelectProps {
    options: BreedOption[]
    selectedIds: string[]
        ...
        switch (event.key) {
            case "ArrowDown":
                ...
                setHighlightedIndex((prev) => {
                    const next = prev < optionsCount - 1 ? prev + 1 : prev
                    setTimeout(() => {
    ...
    return (
        <Popover open={open} onOpenChange={setOpen}>
            <PopoverAnchor asChild>
                <div
                    className={cn(
                        "relative flex min-h-10 w-full flex-wrap items-center gap-2 rounded-md border border-input bg-background px-3 py-2 text-sm shadow-sm transition-colors focus-within:ring-2 focus-within:ring-ring focus-within:ring-offset-2",
                        "cursor-text"
                    )}
                        ...
                        <div className="flex flex-wrap gap-2 flex-1">
                            {selectedOptions.map((option) => (
                                <Badge
                                    key={option.id}
                                    variant="secondary"
                                    className="flex items-center gap-1 text-xs h-6 px-2 cursor-pointer"
                                    onClick={(event) => {
