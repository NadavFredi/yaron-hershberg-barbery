...
export interface SettingsData {
  ...
  breedTypesMap: Record<string, string[]>
...
async function loadBreedsSettingsData(): Promise<SettingsData> {
  ...
  // Fetch all dog types and categories
  const { data: dogTypes, error: dogTypesError } = await supabase.from("dog_types").select("*").order("name")

  if (dogTypesError) {
    console.error("Error fetching dog_types:", dogTypesError)
  }
  ...
  // Fetch breed-dog_types and breed-dog_categories relationships
  const { data: breedTypes, error: breedTypesError } = await supabase
    .from("breed_dog_types")
    .select("breed_id, dog_type_id")

  if (breedTypesError) {
    console.error("Error fetching breed_dog_types:", breedTypesError)
  }
  ...
  // Map breed IDs to their types and categories
  const breedTypesMap: Record<string, string[]> = {}
  const breedCategoriesMap: Record<string, string[]> = {}

  breeds?.forEach((breed) => {
    breedTypesMap[breed.id] = []
    breedCategoriesMap[breed.id] = []
  })

  breedTypes?.forEach((bt) => {
    if (breedTypesMap[bt.breed_id]) {
      breedTypesMap[bt.breed_id].push(bt.dog_type_id)
    }
  })
