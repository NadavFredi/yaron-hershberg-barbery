...
export function SettingsCalendarWindowSection() {
    ...
    const loadSettings = useCallback(async () => {
        ...
        try {
            ...
            if (data) {
                ...
                setSettingsRow({ id: data.id, open_days_ahead: normalized })
                ...
                const { data: inserted, error: insertError } = await supabase
                    .from("calendar_settings")
                    .insert({ open_days_ahead: DEFAULT_DAYS_AHEAD })
                    .select("id, open_days_ahead")
                    .maybeSingle<CalendarSettingsRow>()
                ...
                if (inserted) {
                    ...
                    setSettingsRow({ id: inserted.id, open_days_ahead: normalized })
                    setDaysAhead(normalized)
    ...
    async function handleSave() {
        console.log(
            `[SettingsCalendarWindowSection] Saving calendar settings with open_days_ahead=${daysAhead}, rowId=${settingsRow?.id}`,
        )
        ...
        try {
            if (settingsRow?.id) {
                const { error } = await supabase
                    .from("calendar_settings")
                    .update({ open_days_ahead: normalized })
                ...
                const { data, error } = await supabase
                    .from("calendar_settings")
                    .insert({ open_days_ahead: normalized })
                    .select("id, open_days_ahead")
                    .maybeSingle<CalendarSettingsRow>()
                ...
                if (data) {
                    setSettingsRow({ id: data.id, open_days_ahead: normalizeDaysAhead(data.open_days_ahead) })
                }
            ...
            toast({
                title: "הצלחה",
                description: "טווח פתיחת היומן נשמר בהצלחה.",
            })
    ...
    const hasChanges = settingsRow ? settingsRow.open_days_ahead !== normalizeDaysAhead(daysAhead) : true
